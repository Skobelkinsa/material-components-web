...
You can enhance the button to have ripple effects by instantiate `MDCRipple` on
the `button` element. See [MDC Ripple](https://github.com/material-components/material-components-web/tree/master/packages/mdc-ripple) and [Demo](https://material-components-web.appspot.com/button.html) for detail.
...
the button to `disabled` to disable a button. Disabled buttons cannot be interacted with
...
Button have a default baseline color, but it could also be set to adopt the
 application's primary and accent color by adding the `mdc-button--primary` or
